...
export default function CommentaryListPage() {
  ...
  async function load() {
    ...
    try {
      ...
      console.log("Fetched commentaries:", data);
  ...
  async function removeCommentary(row) {
    if (!window.confirm("Supprimer ce commentaire ?")) return;
    try {
      const res = await fetch(`${API_BASE}/commentaries/${row.id}`, { method: "DELETE" });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
                  ...
                  pageRows.map(row => (
                    <tr key={row.commentary.id}>
                      ...
                      <td className="row-actions">
                        <button className="btn sm" onClick={() => openEdit(row.commentary.id)}>Voir/Éditer</button>
                        <button className={`btn sm ${row.commentary.approved ? "danger" : "success"}`} onClick={() => toggleApprove(row.commentary)}>
                          {row.commentary.approved ? "Unapprove" : "Approve"}
                        </button>
                        <button className="btn sm ghost" onClick={() => removeCommentary(row)}>Supprimer</button>
                      </td>
                    </tr>
                  ))
                )}
