...
export default function CommentaryListPage() {
  // --------------------- State ---------------------
    ...
    const [availableVerses, setAvailableVerses] = useState([]); // [{id, number}, ...] d’un chapitre
    const [verseInput, setVerseInput] = useState("");           // "1-3,5"
    const [verseError, setVerseError] = useState("");           // message d’erreur local
    ...
    async function collectVerses(row) {
        // Déterminer le bookCode & chapterNum depuis le 1er verset
        const first = (row?.verses || [])[0];
        console.log('first', first);
        const bookId = first.bookId;
        const chapterNum = first.chapterNum;

        console.log(`Collecting verses for ${bookId} ${chapterNum}`);
        try {
            const r2 = await fetch(`${API_BASE}/books/${bookId}/${chapterNum}/verses`);
            if (r2.ok) {
                const chapterVerses = await r2.json(); // attendu: [{ id, number, ...}, ...]
                console.log('chapterVerses', chapterVerses);
                // On ne garde que id + number
    ...
    async function openEdit(id) {
        try {
            ...
            collectVerses(row);
            setVerseInput("");  // reset du champ d'édition de versets
            setVerseError("");
        } catch (e) {
            showToast("Impossible d’ouvrir le commentaire", "error");
            console.error(e);
        }
    }
    ...
    async function applyVerses() {
        ...
        // Convertir numéros -> ids via availableVerses
        const mapByNum = new Map(availableVerses.map(v => [v.number, v.id]));
        const verse_ids = [];
        for (const n of nums) {
            const id = mapByNum.get(n);
            if (!id) {
        ...
        try {
            ...
            const res = await fetch(`${API_BASE}/commentaries/${id}`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                // Tu peux aussi inclure { text: textDraft } si tu veux sauver le texte en même temps
            ...
            setRows(prev => prev.map(r =>
                ...
                ? {
                    ...
                    commentary: {
                        ...r.commentary,
                        title: updated.title,
                        text: updated.text ?? r.commentary.text,
                        approved: updated.approved,
                        updatedAt: updated.updatedAt
                    },
                    verses: updated.verses ?? r.verses
            ...
            setEditing(prev =>
                ...
                commentary: {
                    ...prev.commentary,
                    title: updated.title,
                    text: updated.text ?? prev.commentary.text,
                    approved: updated.approved,
                    updatedAt: updated.updatedAt,
                },
                verses: updated.verses ?? prev.verses
            ...
            showToast("Versets mis à jour", "success");
            setVerseInput("");
        } catch (e) {
            console.error(e);
            showToast("Échec de la mise à jour des versets", "error");
        }
    }
      ...
      {editing && (
        <Modal title={`Commentary #${editing.commentary.id}`} onClose={() => setEditing(null)}>
          <div className="form">
            ...
            <label>Ajouter / Remplacer des versets (du même chapitre)</label>
            <div className="row">
                <input
                    value={verseInput}
                    ...
                    style={{ flex: 1 }}
                />
                <button className="btn sm" onClick={applyVerses} style={{ marginLeft: 8 }}>
...
// Vérifie contiguïté stricte (ex: [3,4,5] OK; [3,5] KO)
