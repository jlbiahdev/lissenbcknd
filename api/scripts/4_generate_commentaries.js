 ...
 *   MAX_VERSES_PER_COMMENTARY=3         // taille maximale d'un groupe
...
// Récupère le nom du livre et le numéro du chapitre pour formater le titre
...
function* groupConsecutiveByIdenticalThemes(verses) {
  ...
  while (i < n) {
    ...
    // démarrer seulement si libre + au moins 1 thème
    if (cur.hasCommentary || cur.themeIds.length === 0) { i += 1; continue; }

    const baseSet = cur.themeIds; // trié
    let group = [cur];
    let j = i + 1;
...
async function processChapter(chapterId) {
  ...
  for (const group of groups) {
    ...
    const title = `${bookName} ${chapterNumber}:${range}`;

    await createCommentaryWithLinks({
      title,
      verseIds: stillFree.map(v => v.verseId),
    });
    created += 1;
  }
